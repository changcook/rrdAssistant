		...
		jQuery.ajax({
			...
			success : function(data){
				...
				if(finishrate && finishrate[1]!="100"){
					...
					$('#hwform').css({width:'600px','background':'white'});
					...
					if(( _rate>14&&_month<=12) || (_rate>=14&&_month<=6)){
						console.debug('符合条件 ');
...
notify(" 哈途提示有标！rate:"+rates[1]+"% \" month:"+months[1]+" \" 完成"+finishrate[1]+"%");
							...
							//获取验证码
							// $('#bidAmount').after('<img id="imgyzm" src="'+siteurl+'image.jsp" />' );
							// $('#bidAmount').after('<span id="yzm"></span>');
							//解析验证码
							// var _t = $('#imgyzm').attr('src');
							// $.ajax({
							// 	url : 'http://wt.me/_test/test.php',
							// 	type : 'POST',
							// 	success: function(dddata){
							// 		$('#yzm').html(dddata);
							// 	}
							// })
		...
		// setTimeout( "gotoBigloan()", global_options.frequency );
		// setTimeout( "gotoBigloan()", 3000 );
